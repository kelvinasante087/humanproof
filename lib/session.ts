/** Account-bound, environment-bound HMAC sessions. New sessions contain only a salted
 * fingerprint. Raw-nullifier sessions remain readable for legacy staging migration.
 * Keep the signing secret server-only. */
...
type SessionPayload = { n?: string; h?: string; p?: string; e?: string; iat: number; exp: number };
...
export function readSession(token: string | undefined | null): Session | null {
  ...
  if (Boolean(nullifier) === Boolean(nullifierHash)) return null;
  ...
  if ((payload.e ?? "staging") !== WORLD_ENV) return null;
